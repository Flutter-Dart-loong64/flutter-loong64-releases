...
- `Flutter QEMU Loong64 Release` builds Debian 13 Loong64 archives from the maintained `loong64-main` source lines by default. The workflow uses `Flutter-Dart-loong64/flutter` `loong64-main` for the SDK tree and `Flutter-Dart-loong64/sdk` `loong64-main` for Dart. The scheduled run skips if the matching GitHub Release already has complete Loong64 assets. If a release exists but is missing assets, the workflow uploads only missing files and keeps existing assets unchanged. Manual runs can override `flutter_ref` and `dart_ref`; clear `dart_ref` only when intentionally testing the older upstream Dart tag patch flow. If that patch flow cannot apply cleanly, or if no archives are produced, the workflow fails instead of reporting a successful publish. Set the repository variable `LOONG64_QEMU_IMAGE` if the default container image is not suitable. Publishing uses version-only Git tags; asset filenames keep the `loong64` architecture marker.
